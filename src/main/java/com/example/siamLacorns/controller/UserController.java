...
public class UserController {
    ...
    // УДАЛИТЬ эти методы - они дублируют функциональность UserCollectionController
    ...
    // Оставить только этот метод для получения коллекций пользователя
    @GetMapping("/{userId}/my-collections")
    @PreAuthorize("isAuthenticated() ")
    public ResponseEntity<List<CollectionDTO>> getUserCollections(@PathVariable Long userId) {
        try {
            ...
            logger.error("Error getting user collections: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
}